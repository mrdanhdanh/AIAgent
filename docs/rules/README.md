...
> Mỗi Rule là một **Policy** (không chỉ tài liệu) — có `policy_type`/`severity`/`enforcement`.
...
| RULE-004 | `RULE-004-execution.md` | Execution | Execution theo sequence chuẩn + State Machine |
...
| RULE-007 | `RULE-007-event.md` | Event | Mọi thay đổi phát Event; Event immutable + Taxonomy |
| RULE-008 | `RULE-008-security.md` | Security | Quyền tối thiểu theo thành phần + Permission Model |
...
| RULE-013 | `RULE-013-deterministic-execution.md` | Reliability | Deterministic execution + Determinism Definition |
| RULE-014 | `RULE-014-observability-contract.md` | Observability | Runtime bắt buộc phát telemetry |
| RULE-015 | `RULE-015-backward-compatibility.md` | Compatibility | Mỗi thay đổi khai báo backward compatibility |
...
- **`architecture-registry.yaml`** — **Architecture Registry**: 9 layers (level/depends_on/provides/consumes/owns/emits/principles/rules) + allowed_dependencies + forbidden_dependencies + ownership + interactions + term_layer map. Doctor/Dashboard đọc trực tiếp, sinh Dependency Graph/Layer Diagram.
- **`INDEX.yaml`** — 15 rules + category + dependencies. Doctor kiểm tra vi phạm, Dashboard hiển thị tuân thủ.
- **`rules.schema.json`** — validate rule template (Policy).
