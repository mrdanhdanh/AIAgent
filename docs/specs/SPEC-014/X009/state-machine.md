---
name: spec-014-x009-state-machine
description: SPEC-014 X009 - Dashboard State Machine. 6 states, 5 transitions, 3 guards.
agent: general
---
...
> **Dashboard chuyen trang thai nhu the nao?**

## XS001 - Philosophy

- Dashboard luon co dung mot State.
- State chi thay doi qua Transition (S009).
- Moi Transition sinh Event (S011).
- **Dashboard chay dung State Machine cua Runtime (S009) - khong dinh nghia lai.**

## XS002 - Principles

- State la nguon su that cua Dashboard.
...
| From -> To | Event | Guard |
|-----------|-------|-------|
| XST-001 -> XST-002 | DASHBOARD_RENDERING | View hop le |
| XST-002 -> XST-003 | DASHBOARD_RENDERED | Render xong |
| XST-002 -> XST-006 | DASHBOARD_FAILED | Render fail |
| XST-003 -> XST-004 | DASHBOARD_FILTERED | Filter xong |
| XST-004 -> XST-005 | DASHBOARD_ARCHIVED | Het retention |
...
Guard fail -> BLOCK + DASHBOARD_GUARD + Event (S011).

## XS008 - Events (5)

- Moi transition sinh dung mot event DASHBOARD_*.
- Event immutable, append-only (P010).
- Event co correlation_id (S011).

## XS009 - Registered Machines

- XSTM-001 Dashboard Pipeline (6 states, 5 transitions).

## XS010 - Validation

- initial_state ton tai, moi state reachable.
- Terminal chi co inbound.
- Validate bang Doctor (X019) + Dashboard Test.

## XS011 - Metrics

- dashboard_states_total, dashboard_transitions_total.
- dashboard_failed_total, dashboard_archived_total.
