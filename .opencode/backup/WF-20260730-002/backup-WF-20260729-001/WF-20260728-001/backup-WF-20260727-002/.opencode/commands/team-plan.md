...
steps:
  - order: 1
    ...
    depends_on: []                         # (MỚI) Các step phải chạy trước step này
    validation_command: "dotnet build"     # (MỚI) Lệnh validate cụ thể cho step
per_step_validation:
  - step: 1
...
7. Thứ tự ưu tiên: không gây xung đột, dễ kiểm tra giữa chừng
8. **Quy tắc file không tồn tại**:
...
9. Có rollback strategy cho mỗi bước rủi ro
10. **Tách validate theo giai đoạn**:
...
11. Kết thúc bằng `final_validation` — tránh đến cuối mới phát hiện lỗi dây chuyền
12. Nếu cần tạo file mới — ghi rõ đường dẫn tuyệt đối
13. Nếu cần sửa file — ghi rõ dòng số hoặc pattern
14. **Ràng buộc file**: chỉ liệt kê file thực sự cần sửa — không thừa, không thiếu
