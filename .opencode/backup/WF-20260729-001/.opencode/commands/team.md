...
```yaml
workflow:
  step: 1-9                          # Bước hiện tại (1-8 + complete)
  step_name: analyze|plan|review|backup|build|testplan|test|report|self_improve|complete
  ...
  retry:
    ...
    max_review: 3
    max_test: 3
    self_improve_count: 0-1          # Self-improvement chỉ chạy 1 lần
  ...
  current_data:
    ...
    self_improve_result: null       # Kết quả self-improvement
```
...
### Self-Improvement
{current_data.self_improve_result hoặc "Không thực hiện (workflow không PASS)"}
```

---

### Bước 8: Self-Improvement
**Agent:** `self-improver` (gọi trực tiếp, không qua command riêng)

**Điều kiện:** Chỉ chạy nếu workflow kết thúc với PASS (không chạy nếu blocked/failed)

**Prompt:**
```
Bạn là Self-Improver Agent. Đọc toàn bộ quá trình workflow vừa hoàn tất và phân tích để cải tiến.

Yêu cầu gốc: $ARGUMENTS
...
Hãy phân tích và cập nhật knowledge base theo quy trình trong agent definition của bạn.
Output báo cáo self-improvement.
```

**Sau output:** Lưu `current_data.self_improve_result = output`, set `step = 9`, `step_name = complete`

**Kiểm tra:**
- Output phải chứa ## BÁO CÁO SELF-IMPROVEMENT header
- Nếu output không đúng format → log warning nhưng không block workflow (self-improvement là optional)
- Nếu self-improver không available → bỏ qua, log "Self-improver không available, bỏ qua bước 8"

**Edge case:**
- Nếu workflow thất bại (blocked/failed) → BỎ QUA bước 8, xuống COMPLETE
- Nếu self-improver trả về lỗi → không ảnh hưởng workflow, chỉ log lỗi
...
self_improve:
  workflow == PASS: → self_improve → complete
...
| 8 | (gọi từ team.md) | self-improver | `.opencode/agents/self-improver.md` |
