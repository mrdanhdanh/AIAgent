...
- skill_id: SK-009
  name: "Dev Agent Team workflow orchestration"
  category: "process"
  source_workflow: "Fix 14 skipped bUnit tests - JS interop giới hạn"
  usage_count: 1
  confidence: "new"
  related_files: [".opencode/skills/dev-team/SKILL.md"]
  notes: "Orchestrator điều phối 6 agent qua 8 bước: Analyze → Plan → Review → Backup → Build → TestPlan → Test → Self-Improve. Dùng biến trạng thái workflow.step/step_name/status/retry. Review loop và test-fix loop tối đa 3 lần."
...
- skill_id: SK-010
  ...
  notes: "Playwright E2E test cho Blazor WASM. Cần app chạy trên port 5173 trước. Browser path hardcode trong PlaywrightFixture.cs — cần chỉnh sửa khi chạy trên máy khác. Test file trong JapaneseLearner.E2ETests/."
