...
# Validator cho SPEC-014 — Dashboard
# Checks X1-001..005 (X001 vision, cau truc, traceability) + X2-001..005 (sections)
# ASCII-only (PS 5.1 ANSI). Exit 0 = PASS.
<#
.SYNOPSIS
  Validate SPEC-014 Dashboard.
.DESCRIPTION
  Kiem tra SPEC-014: README, X001-vision.md, SPEC.yaml va cac section X002-X020.
  Exit 0 = PASS.
#>
param([switch]$Silent)
...
$spec7 = Join-Path $root '..\docs\specs\SPEC-014'

if (-not (Test-Path $spec7)) { Write-Error "SPEC-014 not found"; exit 1 }
...
foreach ($f in @('README.md')) {
  if (-not (Test-Path (Join-Path $spec7 $f))) { $errors += "X1-001: missing SPEC-014/$f" }
}
...
if (-not (Test-Path (Join-Path $spec7 'X001-vision.md'))) { $errors += "X1-002: missing X001-vision.md" }
  ...
  foreach ($inv in @('Dashboard doc du lieu TU S011 - khong tao nguon moi (P005).','Dashboard khong thay doi he thong - read-only.','Moi widget dua tren metrics/events S011.','Dashboard khong chua Business Data (S011 OB003A).')) {
...
if (Test-Path $specFile) {
  ...
  if ($st -notmatch '(?m)^id:\s*SPEC-014') { $errors += "X1-005: SPEC.yaml id phai la SPEC-014" }
...
$required = @{
  ...
  'X008' = @('data-model.md','dashboard-data-model.yaml','dashboard-data.schema.json')
  'X009' = @('state-machine.md','dashboard-states.yaml','dashboard-transitions.yaml','Dashboard.schema.json')
  'X010' = @('execution-flow.md','dashboard-execution-flow.yaml','dashboard-execution-flow.schema.json')
  'X011' = @('observability.md','dashboard-events.yaml','dashboard-observability.schema.json')
  'X012' = @('policies.md','dashboard-policies.yaml','dashboard-policies.schema.json')
  'X013' = @('governance.md','dashboard-governance.yaml','dashboard-governance.schema.json')
  'X014' = @('registry.md','dashboard-registry.yaml','dashboard-registry.schema.json')
  'X015' = @('resources.md','dashboard-resource-model.yaml','dashboard-resources.schema.json')
  'X016' = @('compliance.md','dashboard-compliance.yaml','dashboard-compliance.schema.json')
  'X017' = @('extensions.md','dashboard-extensions.yaml')
  'X018' = @('evolution.md','dashboard-versioning.yaml')
  'X019' = @('doctor.md','dashboard-doctor-checks.yaml')
  'X020' = @('dashboard.md','dashboard-panels.yaml')
}
foreach ($d in $required.Keys) {
  foreach ($f in $required[$d]) {
    if (-not (Test-Path (Join-Path $spec7 (Join-Path $d $f)))) { $errors += "X2-001: missing SPEC-014/$d/$f" }
  }
}

# ---------- X2-002: appendix dashboard-models ----------
$cm = Join-Path $spec7 'dashboard-models'
foreach ($f in @('README.md','dashboard-models.yaml','dashboard-models.schema.json')) {
  if (-not (Test-Path (Join-Path $cm $f))) { $errors += "X2-002: missing dashboard-models/$f" }
}
...
Assert-Contains (Join-Path $spec7 'X008\dashboard-entities.yaml') '(?m)^entities:' 'X008'
Assert-Contains (Join-Path $spec7 'X009\dashboard-states.yaml') '(?m)^initial_state:' 'X009'
Assert-Contains (Join-Path $spec7 'X010\dashboard-execution-flow.yaml') '(?m)^stages:' 'X010'
Assert-Contains (Join-Path $spec7 'X011\dashboard-events.yaml') '(?m)^definition_events:' 'X011'
Assert-Contains (Join-Path $spec7 'X012\dashboard-policies.yaml') '(?m)^policies:' 'X012'
Assert-Contains (Join-Path $spec7 'X016\dashboard-compliance.yaml') '(?m)^compliance_rules:' 'X016'
Assert-Contains (Join-Path $spec7 'X019\dashboard-doctor-checks.yaml') '(?m)^checks:' 'X019'
