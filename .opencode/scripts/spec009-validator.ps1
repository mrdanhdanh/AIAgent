...
# Validator cho SPEC-009 — Contract System
# Checks X1-001..005 (X001 vision, cau truc, traceability) + X2-001..005 (sections)
# ASCII-only (PS 5.1 ANSI). Exit 0 = PASS.
<#
.SYNOPSIS
  Validate SPEC-009 Contract System.
.DESCRIPTION
  Kiem tra SPEC-009: README, X001-vision.md, SPEC.yaml va cac section X002-X020.
  Exit 0 = PASS.
#>
param([switch]$Silent)
...
$spec7 = Join-Path $root '..\docs\specs\SPEC-009'

if (-not (Test-Path $spec7)) { Write-Error "SPEC-009 not found"; exit 1 }
...
foreach ($f in @('README.md')) {
  if (-not (Test-Path (Join-Path $spec7 $f))) { $errors += "X1-001: missing SPEC-009/$f" }
}
...
if (-not (Test-Path (Join-Path $spec7 'X001-vision.md'))) { $errors += "X1-002: missing X001-vision.md" }
  ...
  foreach ($inv in @('Contract dinh nghia input/output - khong chua implementation (TERM-014).','Contract luon versioned (P004).','Contract backward compatible (XNF-006).','Contract khong chua Business Data (S011 OB003A).')) {
...
if (Test-Path $specFile) {
  ...
  if ($st -notmatch '(?m)^id:\s*SPEC-009') { $errors += "X1-005: SPEC.yaml id phai la SPEC-009" }
...
$required = @{
  ...
  'X008' = @('data-model.md','contract-data-model.yaml','contract-data.schema.json')
  'X009' = @('state-machine.md','contract-states.yaml','contract-transitions.yaml','contract.schema.json')
  'X010' = @('execution-flow.md','contract-execution-flow.yaml','contract-execution-flow.schema.json')
  'X011' = @('observability.md','contract-events.yaml','contract-observability.schema.json')
  'X012' = @('policies.md','contract-policies.yaml','contract-policies.schema.json')
  'X013' = @('governance.md','contract-governance.yaml','contract-governance.schema.json')
  'X014' = @('registry.md','contract-registry.yaml','contract-registry.schema.json')
  'X015' = @('resources.md','contract-resource-model.yaml','contract-resources.schema.json')
  'X016' = @('compliance.md','contract-compliance.yaml','contract-compliance.schema.json')
  'X017' = @('extensions.md','contract-extensions.yaml')
  'X018' = @('evolution.md','contract-versioning.yaml')
  'X019' = @('doctor.md','contract-doctor-checks.yaml')
  'X020' = @('dashboard.md','contract-dashboard-panels.yaml')
}
foreach ($d in $required.Keys) {
  foreach ($f in $required[$d]) {
    if (-not (Test-Path (Join-Path $spec7 (Join-Path $d $f)))) { $errors += "X2-001: missing SPEC-009/$d/$f" }
  }
}

# ---------- X2-002: appendix contract-models ----------
$cm = Join-Path $spec7 'contract-models'
foreach ($f in @('README.md','contract-models.yaml','contract-models.schema.json')) {
  if (-not (Test-Path (Join-Path $cm $f))) { $errors += "X2-002: missing contract-models/$f" }
}
...
Assert-Contains (Join-Path $spec7 'X008\contract-entities.yaml') '(?m)^entities:' 'X008'
Assert-Contains (Join-Path $spec7 'X009\contract-states.yaml') '(?m)^initial_state:' 'X009'
Assert-Contains (Join-Path $spec7 'X010\contract-execution-flow.yaml') '(?m)^stages:' 'X010'
Assert-Contains (Join-Path $spec7 'X011\contract-events.yaml') '(?m)^definition_events:' 'X011'
Assert-Contains (Join-Path $spec7 'X012\contract-policies.yaml') '(?m)^policies:' 'X012'
Assert-Contains (Join-Path $spec7 'X016\contract-compliance.yaml') '(?m)^compliance_rules:' 'X016'
Assert-Contains (Join-Path $spec7 'X019\contract-doctor-checks.yaml') '(?m)^checks:' 'X019'
...
if (-not $Silent) {
  ""
  "=== SPEC-009 Contract System Validation ==="
