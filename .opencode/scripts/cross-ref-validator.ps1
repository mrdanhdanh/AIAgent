...
function Test-CommandReferenced {
    param([string]$Content, [string]$Cmd)
    $variants = @("/$Cmd.md", "/$Cmd", "$Cmd.md", $Cmd)
    foreach ($v in $variants) {
        $pattern = [regex]::Escape($v) + "(?![-\w])"
...
foreach ($agent in $agentNames) {
    $content = Get-Content -LiteralPath "$opencodeDir/agents/$agent.md" -Raw -Encoding UTF8
    foreach ($cmd in $commandNames) {
        if ((Test-CommandReferenced -Content $content -Cmd $cmd) -and -not (Test-Path "$opencodeDir/commands/$cmd.md")) {
            $checks += [PSCustomObject]@{ Check = "agent->command"; From = "$agent.md"; Target = "$cmd.md"; Status = "FAIL"; Message = "References non-existent command" }
            $allPass = $false
        }
    }
}
...
foreach ($cmd in $commandNames) {
    $cmdFile = "$opencodeDir/commands/$cmd.md"
    if (-not (Test-Path $cmdFile)) {
        $checks += [PSCustomObject]@{ Check = "command->agent"; From = "$cmd.md"; Target = "N/A"; Status = "WARN"; Message = "Command file missing - skipped" }
        continue
    }
    $content = Get-Content -LiteralPath $cmdFile -Raw -Encoding UTF8
    # Read agent declaration from YAML frontmatter only (--- ... ---)
    $fmMatch = [regex]::Match($content, '(?s)^---\s*\r?\n(.*?)\r?\n---')
    $fmText = if ($fmMatch.Success) { $fmMatch.Groups[1].Value } else { "" }
    $agentRef = [regex]::Match($fmText, "agent:\s*([\w-]+)")
    if ($agentRef.Success) {
        $agentFile = "$($agentRef.Groups[1].Value).md"
        if (-not (Test-Path "$opencodeDir/agents/$agentFile")) {
            $checks += [PSCustomObject]@{ Check = "command->agent"; From = "$cmd.md"; Target = $agentFile; Status = "FAIL"; Message = "References non-existent agent" }
            $allPass = $false
        }
    }
}
...
$skillContent = Get-Content -LiteralPath "$opencodeDir/skills/dev-team/SKILL.md" -Raw -Encoding UTF8
foreach ($agent in $agentNames) {
