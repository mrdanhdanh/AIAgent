# spec016-validator.ps1
# Validator cho SPEC-016 — CLI
# Checks X1-001..005 (X001 vision, cau truc, traceability) + X2-001..005 (sections)
# ASCII-only (PS 5.1 ANSI). Exit 0 = PASS.
<#
.SYNOPSIS
  Validate SPEC-016 CLI.
.DESCRIPTION
  Kiem tra SPEC-016: README, X001-vision.md, SPEC.yaml va cac section X002-X020.
  Exit 0 = PASS.
#>
param([switch]$Silent)
...
$spec7 = Join-Path $root '..\docs\specs\SPEC-016'

if (-not (Test-Path $spec7)) { Write-Error "SPEC-016 not found"; exit 1 }
...
foreach ($f in @('README.md')) {
  if (-not (Test-Path (Join-Path $spec7 $f))) { $errors += "X1-001: missing SPEC-016/$f" }
}
...
if (-not (Test-Path (Join-Path $spec7 'X001-vision.md'))) { $errors += "X1-002: missing X001-vision.md" }
  ...
  foreach ($inv in @('Command la entry point - chi khoi dong Runtime, khong lam viec (TERM-007).','Command trigger workflow (SPEC-002).','Moi command co help + alias.','CLI khong chua Business Data (S011 OB003A).')) {
...
if (Test-Path $specFile) {
  ...
  if ($st -notmatch '(?m)^id:\s*SPEC-016') { $errors += "X1-005: SPEC.yaml id phai la SPEC-016" }
...
$required = @{
  ...
  'X008' = @('data-model.md','CLI-data-model.yaml','CLI-data.schema.json')
  'X009' = @('state-machine.md','CLI-states.yaml','CLI-transitions.yaml','CLI.schema.json')
  'X010' = @('execution-flow.md','CLI-execution-flow.yaml','CLI-execution-flow.schema.json')
  'X011' = @('observability.md','CLI-events.yaml','CLI-observability.schema.json')
  'X012' = @('policies.md','CLI-policies.yaml','CLI-policies.schema.json')
  'X013' = @('governance.md','CLI-governance.yaml','CLI-governance.schema.json')
  'X014' = @('registry.md','CLI-registry.yaml','CLI-registry.schema.json')
  'X015' = @('resources.md','CLI-resource-model.yaml','CLI-resources.schema.json')
  'X016' = @('compliance.md','CLI-compliance.yaml','CLI-compliance.schema.json')
  'X017' = @('extensions.md','CLI-extensions.yaml')
  'X018' = @('evolution.md','CLI-evolution-metrics.yaml')
  'X019' = @('doctor.md','CLI-doctor-checks.yaml')
  'X020' = @('dashboard.md','CLI-panels.yaml')
}
foreach ($d in $required.Keys) {
  foreach ($f in $required[$d]) {
    if (-not (Test-Path (Join-Path $spec7 (Join-Path $d $f)))) { $errors += "X2-001: missing SPEC-016/$d/$f" }
  }
}

# ---------- X2-002: appendix CLI-models ----------
$cm = Join-Path $spec7 'CLI-models'
foreach ($f in @('README.md','CLI-models.yaml','CLI-models.schema.json')) {
  if (-not (Test-Path (Join-Path $cm $f))) { $errors += "X2-002: missing CLI-models/$f" }
}
...
Assert-Contains (Join-Path $spec7 'X008\CLI-entities.yaml') '(?m)^entities:' 'X008'
Assert-Contains (Join-Path $spec7 'X009\CLI-states.yaml') '(?m)^initial_state:' 'X009'
Assert-Contains (Join-Path $spec7 'X010\CLI-execution-flow.yaml') '(?m)^stages:' 'X010'
Assert-Contains (Join-Path $spec7 'X011\CLI-events.yaml') '(?m)^definition_events:' 'X011'
Assert-Contains (Join-Path $spec7 'X012\CLI-policies.yaml') '(?m)^policies:' 'X012'
Assert-Contains (Join-Path $spec7 'X016\CLI-compliance.yaml') '(?m)^compliance_rules:' 'X016'
Assert-Contains (Join-Path $spec7 'X019\CLI-doctor-checks.yaml') '(?m)^checks:' 'X019'
