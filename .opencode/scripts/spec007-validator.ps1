...
# Validator cho SPEC-007 — Artifact Manager
# Checks X1-001..005 (X001 vision, cau truc, traceability) + X2-001..005 (sections)
# ASCII-only (PS 5.1 ANSI). Exit 0 = PASS.
<#
.SYNOPSIS
  Validate SPEC-007 Artifact Manager.
.DESCRIPTION
  Kiem tra SPEC-007: README, X001-vision.md, SPEC.yaml va cac section X002-X020.
  Exit 0 = PASS.
#>
param([switch]$Silent)
...
$spec7 = Join-Path $root '..\docs\specs\SPEC-007'

if (-not (Test-Path $spec7)) { Write-Error "SPEC-007 not found"; exit 1 }
...
foreach ($f in @('README.md')) {
  if (-not (Test-Path (Join-Path $spec7 $f))) { $errors += "X1-001: missing SPEC-007/$f" }
}
...
if (-not (Test-Path (Join-Path $spec7 'X001-vision.md'))) { $errors += "X1-002: missing X001-vision.md" }
  ...
  foreach ($inv in @('Artifact luon immutable - khong mutable (P010, RULE-005).','Artifact co checksum - khong doi checksum (P010).','Artifact khong bao gio bi overwrite (TERM-008).','Artifact khong chua Business Data (S011 OB003A).')) {
...
if (Test-Path $specFile) {
  ...
  if ($st -notmatch '(?m)^id:\s*SPEC-007') { $errors += "X1-005: SPEC.yaml id phai la SPEC-007" }
...
$required = @{
  ...
  'X008' = @('data-model.md','artifact-data-model.yaml','artifact-data.schema.json')
  'X009' = @('state-machine.md','artifact-states.yaml','artifact-transitions.yaml','artifact.schema.json')
  'X010' = @('execution-flow.md','artifact-execution-flow.yaml','artifact-execution-flow.schema.json')
  'X011' = @('observability.md','artifact-events.yaml','artifact-observability.schema.json')
  'X012' = @('policies.md','artifact-policies.yaml','artifact-policies.schema.json')
  'X013' = @('governance.md','artifact-governance.yaml','artifact-governance.schema.json')
  'X014' = @('registry.md','artifact-registry.yaml','artifact-registry.schema.json')
  'X015' = @('resources.md','artifact-resource-model.yaml','artifact-resources.schema.json')
  'X016' = @('compliance.md','artifact-compliance.yaml','artifact-compliance.schema.json')
  'X017' = @('extensions.md','artifact-extensions.yaml')
  'X018' = @('evolution.md','artifact-versioning.yaml')
  'X019' = @('doctor.md','artifact-doctor-checks.yaml')
  'X020' = @('dashboard.md','artifact-dashboard-panels.yaml')
}
foreach ($d in $required.Keys) {
  foreach ($f in $required[$d]) {
    if (-not (Test-Path (Join-Path $spec7 (Join-Path $d $f)))) { $errors += "X2-001: missing SPEC-007/$d/$f" }
  }
}

# ---------- X2-002: appendix artifact-models ----------
$cm = Join-Path $spec7 'artifact-models'
foreach ($f in @('README.md','artifact-models.yaml','artifact-models.schema.json')) {
  if (-not (Test-Path (Join-Path $cm $f))) { $errors += "X2-002: missing artifact-models/$f" }
}
...
Assert-Contains (Join-Path $spec7 'X008\artifact-entities.yaml') '(?m)^entities:' 'X008'
Assert-Contains (Join-Path $spec7 'X009\artifact-states.yaml') '(?m)^initial_state:' 'X009'
Assert-Contains (Join-Path $spec7 'X010\artifact-execution-flow.yaml') '(?m)^stages:' 'X010'
Assert-Contains (Join-Path $spec7 'X011\artifact-events.yaml') '(?m)^definition_events:' 'X011'
Assert-Contains (Join-Path $spec7 'X012\artifact-policies.yaml') '(?m)^policies:' 'X012'
Assert-Contains (Join-Path $spec7 'X016\artifact-compliance.yaml') '(?m)^compliance_rules:' 'X016'
Assert-Contains (Join-Path $spec7 'X019\artifact-doctor-checks.yaml') '(?m)^checks:' 'X019'
...
if (-not $Silent) {
  ""
  "=== SPEC-007 Artifact Manager Validation ==="
