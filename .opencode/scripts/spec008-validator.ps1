...
# Validator cho SPEC-008 — Event Bus
# Checks X1-001..005 (X001 vision, cau truc, traceability) + X2-001..005 (sections)
# ASCII-only (PS 5.1 ANSI). Exit 0 = PASS.
<#
.SYNOPSIS
  Validate SPEC-008 Event Bus.
.DESCRIPTION
  Kiem tra SPEC-008: README, X001-vision.md, SPEC.yaml va cac section X002-X020.
  Exit 0 = PASS.
#>
param([switch]$Silent)
...
$spec7 = Join-Path $root '..\docs\specs\SPEC-008'

if (-not (Test-Path $spec7)) { Write-Error "SPEC-008 not found"; exit 1 }
...
foreach ($f in @('README.md')) {
  if (-not (Test-Path (Join-Path $spec7 $f))) { $errors += "X1-001: missing SPEC-008/$f" }
}
...
if (-not (Test-Path (Join-Path $spec7 'X001-vision.md'))) { $errors += "X1-002: missing X001-vision.md" }
  ...
  foreach ($inv in @('Event luon immutable - khong mutable (P010).','Event co lineage - khong mat chain (RULE-007).','Event chi duoc append - khong overwrite, khong xoa (P005).','Event khong chua Business Data (S011 OB003A).')) {
...
if (Test-Path $specFile) {
  ...
  if ($st -notmatch '(?m)^id:\s*SPEC-008') { $errors += "X1-005: SPEC.yaml id phai la SPEC-008" }
  if ($st -notmatch '(?m)^implements:') { $errors += "X1-005: SPEC.yaml thieu implements" }
  foreach ($d in @('SPEC-000','SPEC-001','SPEC-005','SPEC-007')) { if ($st -notmatch [regex]::Escape($d)) { $errors += "X1-005: thieu dependency $d" } }
}
...
$required = @{
  ...
  'X008' = @('data-model.md','event-data-model.yaml','event-data.schema.json')
  'X009' = @('state-machine.md','event-states.yaml','event-transitions.yaml','event.schema.json')
  'X010' = @('execution-flow.md','event-execution-flow.yaml','event-execution-flow.schema.json')
  'X011' = @('observability.md','event-events.yaml','event-observability.schema.json')
  'X012' = @('policies.md','event-policies.yaml','event-policies.schema.json')
  'X013' = @('governance.md','event-governance.yaml','event-governance.schema.json')
  'X014' = @('registry.md','event-registry.yaml','event-registry.schema.json')
  'X015' = @('resources.md','event-resource-model.yaml','event-resources.schema.json')
  'X016' = @('compliance.md','event-compliance.yaml','event-compliance.schema.json')
  'X017' = @('extensions.md','event-extensions.yaml')
  'X018' = @('evolution.md','event-versioning.yaml')
  'X019' = @('doctor.md','event-doctor-checks.yaml')
  'X020' = @('dashboard.md','event-dashboard-panels.yaml')
}
foreach ($d in $required.Keys) {
  foreach ($f in $required[$d]) {
    if (-not (Test-Path (Join-Path $spec7 (Join-Path $d $f)))) { $errors += "X2-001: missing SPEC-008/$d/$f" }
  }
}

# ---------- X2-002: appendix event-models ----------
$cm = Join-Path $spec7 'event-models'
foreach ($f in @('README.md','event-models.yaml','event-models.schema.json')) {
  if (-not (Test-Path (Join-Path $cm $f))) { $errors += "X2-002: missing event-models/$f" }
}
...
Assert-Contains (Join-Path $spec7 'X008\event-entities.yaml') '(?m)^entities:' 'X008'
Assert-Contains (Join-Path $spec7 'X009\event-states.yaml') '(?m)^initial_state:' 'X009'
Assert-Contains (Join-Path $spec7 'X010\event-execution-flow.yaml') '(?m)^stages:' 'X010'
Assert-Contains (Join-Path $spec7 'X011\event-events.yaml') '(?m)^definition_events:' 'X011'
Assert-Contains (Join-Path $spec7 'X012\event-policies.yaml') '(?m)^policies:' 'X012'
Assert-Contains (Join-Path $spec7 'X016\event-compliance.yaml') '(?m)^compliance_rules:' 'X016'
Assert-Contains (Join-Path $spec7 'X019\event-doctor-checks.yaml') '(?m)^checks:' 'X019'
...
$specRefs = @('SPEC-000','SPEC-001','SPEC-005','SPEC-007')
foreach ($d in $required.Keys) {
