...
# Validator cho SPEC-012 — Simulation
# Checks X1-001..005 (X001 vision, cau truc, traceability) + X2-001..005 (sections)
# ASCII-only (PS 5.1 ANSI). Exit 0 = PASS.
<#
.SYNOPSIS
  Validate SPEC-012 Simulation.
.DESCRIPTION
  Kiem tra SPEC-012: README, X001-vision.md, SPEC.yaml va cac section X002-X020.
  Exit 0 = PASS.
#>
param([switch]$Silent)
...
$spec7 = Join-Path $root '..\docs\specs\SPEC-012'

if (-not (Test-Path $spec7)) { Write-Error "SPEC-012 not found"; exit 1 }
...
foreach ($f in @('README.md')) {
  if (-not (Test-Path (Join-Path $spec7 $f))) { $errors += "X1-001: missing SPEC-012/$f" }
}
...
if (-not (Test-Path (Join-Path $spec7 'X001-vision.md'))) { $errors += "X1-002: missing X001-vision.md" }
  ...
  foreach ($inv in @('Simulation khong thay doi he thong that - isolated (RULE-007).','Simulation deterministic - cung input cung ket qua.','Simulation replayable qua Event log (RULE-007).','Simulation khong tao Business Data (S011 OB003A).')) {
...
if (Test-Path $specFile) {
  ...
  if ($st -notmatch '(?m)^id:\s*SPEC-012') { $errors += "X1-005: SPEC.yaml id phai la SPEC-012" }
...
$required = @{
  ...
  'X008' = @('data-model.md','Simulation-data-model.yaml','Simulation-data.schema.json')
  'X009' = @('state-machine.md','Simulation-states.yaml','Simulation-transitions.yaml','simulation.schema.json')
  'X010' = @('execution-flow.md','simulation-execution-flow.yaml','simulation-execution-flow.schema.json')
  'X011' = @('observability.md','Simulation-events.yaml','simulation-observability.schema.json')
  'X012' = @('policies.md','simulation-policies.yaml','simulation-policies.schema.json')
  'X013' = @('governance.md','simulation-governance.yaml','simulation-governance.schema.json')
  'X014' = @('registry.md','simulation-registry.yaml','simulation-registry.schema.json')
  'X015' = @('resources.md','simulation-resource-model.yaml','simulation-resources.schema.json')
  'X016' = @('compliance.md','simulation-compliance.yaml','simulation-compliance.schema.json')
  'X017' = @('extensions.md','simulation-extensions.yaml')
  'X018' = @('evolution.md','simulation-versioning.yaml')
  'X019' = @('doctor.md','simulation-doctor-checks.yaml')
  'X020' = @('dashboard.md','simulation-dashboard-panels.yaml')
}
foreach ($d in $required.Keys) {
  foreach ($f in $required[$d]) {
    if (-not (Test-Path (Join-Path $spec7 (Join-Path $d $f)))) { $errors += "X2-001: missing SPEC-012/$d/$f" }
  }
}

# ---------- X2-002: appendix simulation-models ----------
$cm = Join-Path $spec7 'simulation-models'
foreach ($f in @('README.md','simulation-models.yaml','simulation-models.schema.json')) {
  if (-not (Test-Path (Join-Path $cm $f))) { $errors += "X2-002: missing simulation-models/$f" }
}
...
Assert-Contains (Join-Path $spec7 'X008\Simulation-entities.yaml') '(?m)^entities:' 'X008'
Assert-Contains (Join-Path $spec7 'X009\Simulation-states.yaml') '(?m)^initial_state:' 'X009'
Assert-Contains (Join-Path $spec7 'X010\simulation-execution-flow.yaml') '(?m)^stages:' 'X010'
Assert-Contains (Join-Path $spec7 'X011\Simulation-events.yaml') '(?m)^definition_events:' 'X011'
Assert-Contains (Join-Path $spec7 'X012\simulation-policies.yaml') '(?m)^policies:' 'X012'
Assert-Contains (Join-Path $spec7 'X016\simulation-compliance.yaml') '(?m)^compliance_rules:' 'X016'
Assert-Contains (Join-Path $spec7 'X019\simulation-doctor-checks.yaml') '(?m)^checks:' 'X019'
...
if (-not $Silent) {
  ""
  "=== SPEC-012 Simulation Validation ==="
